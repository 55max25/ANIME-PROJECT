...
import { ArrowLeft, Tv, Filter } from 'lucide-react'
...
export async function generateMetadata({ params }: Props) {
  ...
  return {
    title: `Anime ${genreName} - YumeStream`,
    description: `Daftar anime genre ${genreName} dengan subtitle Indonesia di YumeStream`,
  }
}

export default async function AnimeGenrePage({ params }: Props) {
  const { slug } = await params
  const genreName = formatGenreName(slug)

  // Fetch anime by genre
  let animeList: { title: string; slug: string; poster: string; episode?: string; rating?: string }[] = []
  
  try {
    const res = await fetch(`https://www.sankavollerei.com/anime/genre/${slug}`, {
      next: { revalidate: 3600 }
    })
    if (res.ok) {
      const json = await res.json()
      const list = json?.data?.animeList || json?.animeList || json?.data || []
      animeList = Array.isArray(list) ? list.map((item: Record<string, unknown>) => ({
        title: String(item.title || ''),
        slug: String(item.animeId || item.slug || ''),
        poster: String(item.poster || item.thumbnail || ''),
        episode: item.episodes ? `Ep ${item.episodes}` : item.episode ? String(item.episode) : undefined,
        rating: item.rating ? String(item.rating) : undefined,
      })) : []
    }
  } catch (error) {
    console.error('[v0] Error fetching anime by genre:', error)
  }
          ...
          <div className="relative flex flex-col items-start gap-4 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <h1 className="mb-3 text-3xl font-bold md:text-4xl">
                Anime <span className="gradient-text">{genreName}</span>
              </h1>
              <p className="max-w-xl text-foreground/70">
                Koleksi anime dengan genre {genreName} lengkap dengan subtitle Indonesia
              </p>
            </div>
            
            <div className="glass rounded-xl px-5 py-3 text-center">
              <div className="text-2xl font-bold gradient-text">{animeList.length}</div>
        ...
        {animeList.length > 0 ? (
          ...
          <div className="flex flex-col items-center justify-center rounded-2xl bg-card/50 py-20 text-center backdrop-blur-sm">
            ...
            <p className="text-sm text-muted-foreground">
              Coba cari genre lain atau kembali ke halaman anime
            </p>
          </div>
        )}
      </main>
